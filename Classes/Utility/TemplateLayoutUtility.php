...
class TemplateLayoutUtility {
	...
	private static function getTemplateLayoutsFromTsConfig($pageUid, $key) {
		...
		$path = ExtensionUtility::extensionSignature() . './templateLayouts./' . $key . '.';

		$templateLayouts = ArrayUtility::getValueByPath($pagesTsConfig, $path, '/');
		return (array) $templateLayouts;
	}
}